...
def compute_rfe_inertial_parameters(
    rfe_lengths: jnp.ndarray, 
    dlo_params: DLOParameters
...
def compute_rfe_sde_parameters(delta_l: int, dlo_params: DLOParameters):
    """
    NOTE Torsion stiffness is not considered
    """
    d = dlo_params.diameter_inner
    D = dlo_params.diameter_outer
    ...
    # We are also interested in bending stiffness, nor torsion
    sde_k = [spring_params_hollow_cylinder(d, D, delta_l, E, G)[1:,:]]
    sde_d = [spring_params_hollow_cylinder(d, D, delta_l, nu, nu_bar)[1:,:]]
    return sde_k, sde_d


def compute_rfe_sde_parameters_for_uniform_discretization(
    n_seg: int, dlo_params: DLOParameters
):
    L = dlo_params.length
    delta_l = L/n_seg
    sde_ki, sde_di = compute_rfe_sde_parameters(delta_l, dlo_params)
    return sde_ki*n_seg, sde_di*n_seg
...
def inertial_params_hollow_cylinder(d: float, D: float, l: float, rho):
    ...
    Ixx = jnp.pi*rho*l/12*(3*(ro**4 - ri**4) + l**2*(ro**2 - ri**2))
    Iyy = jnp.pi*rho*l/12*(3*(ro**4 - ri**4) + l**2*(ro**2 - ri**2))
    Izz = jnp.pi*rho*l/2*(ro**4 - ri**4)
